...
development = data[data["cluster_split"] == "train"].reset_index(drop=True)
cv = StratifiedGroupKFold(n_splits=3, shuffle=True, random_state=SEED)
folds = list(cv.split(development["peptide"], development["label_hiconf"], development["edit_distance_2_cluster"]))

# Fit each fold's vectorizer once, then reuse matrices for every candidate.
fold_matrices = []
for fold_id, (train_idx, valid_idx) in enumerate(folds):
    vec = vectorizer()
    x_train = vec.fit_transform(development.loc[train_idx, "peptide"])
    x_valid = vec.transform(development.loc[valid_idx, "peptide"])
    fold_matrices.append((train_idx, valid_idx, x_train, x_valid))
...
for model_name, grid in grids.items():
    for candidate_id, params in enumerate(grid):
        ...
        for train_idx, valid_idx, x_train, x_valid in fold_matrices:
            y_train = development.loc[train_idx, "label_hiconf"].to_numpy(dtype=int)
            y_valid = development.loc[valid_idx, "label_hiconf"].to_numpy(dtype=int)
            positive_weight = float((y_train == 0).sum() / (y_train == 1).sum())
            model = estimator(model_name, params, positive_weight)
            model.fit(x_train, y_train)
            fold_scores.append(float(average_precision_score(y_valid, model.predict_proba(x_valid)[:, 1])))
        search_rows.append(
            {
                "model": model_name,
        ...
        print("search", model_name, params, np.mean(fold_scores))

search = pd.DataFrame(search_rows)
search.to_csv(OUT / "hyperparameter_search.csv", index=False)
best_params = {}
for model_name in grids:
    best_row = search[search["model"] == model_name].sort_values(
        ["mean_group_cv_pr_auc", "candidate_id"], ascending=[False, True]
    ).iloc[0]
...
with (OUT / "selected_parameters.json").open("w", encoding="utf-8") as stream:
    json.dump(best_params, stream, indent=2, sort_keys=True)
print("selected", best_params)

metrics_rows, prediction_frames, efficiency_rows = [], [], []
...
with (OUT / "provenance.json").open("w", encoding="utf-8") as stream:
    json.dump(
        {
            "selection_dataset": "cluster-disjoint training partition only",
            "selection_metric": "mean 3-fold StratifiedGroupKFold PR-AUC",
